...
void draw() {
  ...
  for (uint8_t sliceIndex = 0; sliceIndex < slices; sliceIndex++) {
    ...
    */
    ...
    // draw octopus legs
    for(size_t index = 0; index < octo[0].currentLength; ++index){
      drawSprite(spriteOctopusLeg1[index], sliceY, buffer);
    }
  
    for(size_t index = 0; index < octo[1].currentLength; ++index){
      drawSprite(spriteOctopusLeg2[index], sliceY, buffer);
    }

    for(size_t index = 0; index < octo[2].currentLength; ++index){
      drawSprite(spriteOctopusLeg3[index], sliceY, buffer);
    }

    for(size_t index = 0; index < octo[3].currentLength; ++index){
